...
def make_auditor_chain(model_config: Dict[str, Any]):
    ...
    prompt = PromptTemplate.from_template(
        ...
        **CRITICAL: Your internal thinking/reasoning process MUST also be in the SAME LANGUAGE as the input.**
        ...
        你是监察官（质疑者）。收到议长的“质疑审核”指令后，针对每个方案给出逐项质疑、改进建议与评级。
...
def make_reporter_chain(model_config: Dict[str, Any]):
    ...
    prompt = PromptTemplate.from_template(
        ...
        **核心要求**：
        ...
        5. **交互性**：可以在 HTML 中加入简单的 JS（如点击展开详情、切换标签等）以增强展示效果。
        6. **结构遵循**：请务必遵循议长设计的报告结构（report_design）进行内容组织。
        7. **语言一致性**：报告的所有内容（包括标题、按钮、标签、正文）必须使用与原始议题相同的语言。
        8. **引用与参考资料（严禁虚构链接）**：
        ...
        9. **禁止废话**：不要包含任何关于报告生成过程的描述（如“基于多轮讨论形成”、“本报告整合自...”）、版权声明、讲解时长建议或任何前言/后记。直接从报告标题和正文内容开始。
